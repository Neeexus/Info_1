...
---
# Real-Time PCR을 통한 양성 판정 분별
![pcr1.PNG](./img/pcr1.PNG)
![pcr2.PNG](./img/pcr2.PNG)
![pcr3.PNG](./img/pcr3.PNG)
![pcr4.PNG](./img/pcr4.PNG)
![pcr5.PNG](./img/pcr5.PNG)
![pcr6.PNG](./img/pcr6.PNG)
![pcr7.PNG](./img/pcr7.PNG)

### 기능 설명
- 해당 프로젝트는 최근 확산되고 있는 코로나로 인해 Real-Time PCR을 이용한 POCT 장비의 농도판정 정확도를 향상시키는 목적으로 진행하였다.
- 사진과 같이 PCR 칩을 직접 만들어 사용했으며, 직접 만들다 보니 데이터가 적었다.
- 시약의 용량과 프로토콜은 사진과 같다.
- 마지막 과정을 통해 기존 방식의 ANOVA를 비교 했을 때, Sigmoid 함수를 사용한 모델이 기존 방식인 Log Threshold 방식보다 값이 더 커 Sigmoid 함수를 최종 결과 모델로 선정하였다.
- 적은 양의 데이터를 사용한 만큼 기준이 정확하지 못하다. 이후 데이터 양을 늘려 농도 판정 기준뿐만아니라 양성 판정의 기준을 세워 실사용에 적용하는 것을 목표로 하고있다.
